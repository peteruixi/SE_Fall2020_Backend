 ...
 */
public class OracleTemplateImpl extends BaseSqlTemplateImpl {
    ...
    public  <T> Page queryList(Page page,String sql ,Class<T> entityClass,
       ...
       // sql=sql+" Limit "+ limit+","+page.getmPageSize();
        sql="select * from (select   tttt_temp_ttt.*,rownum rn_gyip_rn_temp from  ( "+sql+")  tttt_temp_ttt) where   rn_gyip_rn_temp  between  " + (limit+1) + "  and "+(limit+page.getmPageSize());
